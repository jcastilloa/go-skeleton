...
- **Dependency injection** (API template) via [sarulabs/di](https://github.com/sarulabs/di)
- **Single binary, zero runtime dependencies** — the scaffold tool compiles to a standalone Go binary

## Prerequisites

- **Go 1.22+** (the scaffold tool itself requires Go; generated projects target Go 1.22+)
...
Generates a Cobra + Viper command-line application with OpenAI integration.
...
├── platform/
│   ├── cli/commands/
│   │   └── root.go                    # Root command with prompt flags
│   ├── config/
│   │   └── viper_repository.go        # Viper-based config reader
│   └── openai/
│       └── openai_repository.go       # OpenAI-compatible AI provider
...
| `--service`| No       | Service name for config      |
...
Seven curated skill files are installed into `.codex/skills/` with symlinks at `.claude/skills/` and `.opencode/skills/`:
